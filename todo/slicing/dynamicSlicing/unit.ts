...
export class Unit {
    ...
    async startBuild() {
        ...
        const savedDisaster = await db.disasterDB.getWithRange(this.latitude - this.radius, this.longitude - this.radius, this.latitude + this.radius, this.longitude + this.radius);
    ...
    async getOrcreateDisaster(event: Post) {
        ...
        if (sharedDisaster.length) {
            ...
            startTime = Date.now();
            await db.disasterDB.createDisaster(mongodbDisaster);
